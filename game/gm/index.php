...
if ($upass == '') {
		echo "<script>alert('请输入后台密码');history.go(-1)</script>";exit;	
		}
	...
	if (!$vipjson[$uid]) {
		...
		echo "<script>alert('后台密码不匹配');history.go(-1)</script>";exit;
	}
      ...
      <div class="modal-body">
			...
			<div class="alert alert-danger">
                ...
                <div class="form-group">
				 ...
				 <hr/>
  <div>
  <input type='text' value='' name="name" id='uid' class="form-control" placeholder='请输入<?php echo $xname; ?>'>  
  <input type='text' value='' name="pwd" id='upass' class="form-control" placeholder='请输入后台密码'>
  <hr/>
 <div>
 <input type='submit'class="btn btn-danger btn-block"  value='登陆后台'  name="sub"  id='chargebtn'>
 </div> 
  <hr/>
</form>
</div>
